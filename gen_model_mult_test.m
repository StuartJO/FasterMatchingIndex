%function [B,b] = gen_model_mult_test(A,PD,m,modeltype,modelvar,PDexpo,gam,epsilon)
...
for ii = (mseed + 1):m
    ...
    for i = 1:length(updateuu)
        ...
        if (ncon==0)
            ...
            K1(uu,j) = (2*(sum(c1(use)&c2(use))/ncon)) + epsilon;
            K1(j,uu) = K1(uu,j);
        end
    ...
    for i = 1:length(updatevv)
        ...
        if (ncon==0)
            ...
            K1(vv,j) = (2*(sum(c1(use)&c2(use))/ncon)) + epsilon;
            K1(j,vv) = K1(vv,j);
        end
    end
    switch mv2
